...
const adminDB = {
    doctors: [
        { id: "2001", name: "د. أحمد", specialty: "علاج طبيعي", patientsCount: 12, status: "نشط" },
        { id: "2002", name: "د. سارة محمود", specialty: "إصابات ملاعب", patientsCount: 8, status: "نشط" },
        { id: "2003", name: "د. كريم عادل", specialty: "علاج طبيعي للأطفال", patientsCount: 5, status: "معلق" }
    ],
    secretaries: [
        { id: "3001", name: "منة الله سعيد", phone: "01011122233", joinDate: "01 / 03 / 2026", status: "نشط" },
        { id: "3002", name: "ياسمين طارق", phone: "01099988877", joinDate: "15 / 05 / 2026", status: "نشط" }
    ],
    patients: [
...
function updateSearchPlaceholder() {
    switch (state.currentPage) {
        case "doctors": globalSearch.placeholder = "ابحث باسم الدكتور أو التخصص"; break;
        case "secretaries": globalSearch.placeholder = "ابحث باسم السكرتير"; break;
        case "patients": globalSearch.placeholder = "ابحث باسم المريض أو رقم الملف"; break;
        case "sessions": globalSearch.placeholder = "ابحث عن جلسة"; break;
        case "appointments": globalSearch.placeholder = "ابحث عن موعد"; break;
        default: globalSearch.placeholder = "ابحث...";
    }
}
...
function renderDoctors(data = adminDB.doctors) {
    ...
    data.forEach(doctor => {
        ...
        <tr>
            ...
            <td><span class="status ${getStatusClass(doctor.status)}">${doctor.status}</span></td>
            <td>
                <button class="primary-btn open-doctor-btn" data-id="${doctor.id}">عرض</button>
            </td>
        </tr>
        `;
    });
...
function renderSecretaries(data = adminDB.secretaries) {
    ...
    data.forEach(secretary => {
        ...
        <tr>
            ...
            <td><span class="status ${getStatusClass(secretary.status)}">${secretary.status}</span></td>
            <td>
                <button class="primary-btn open-secretary-btn" data-id="${secretary.id}">عرض</button>
            </td>
        </tr>
        `;
    });
...
function renderPatients(data = adminDB.patients) {
    ...
    data.forEach(patient => {
        ...
        <tr>
            ...
            <td>
                <button class="primary-btn open-patient-btn" data-id="${patient.id}">فتح الملف</button>
            </td>
        </tr>
        `;
    });
...
function renderSessions(data = adminDB.sessions) {
    ...
    data.forEach(session => {
        ...
        <tr>
            ...
            <td>
                <button class="primary-btn open-patient-btn" data-id="${session.id}">فتح الملف</button>
            </td>
        </tr>
        `;
    });
...
document.addEventListener("click", function (event) {
    ...
    if (doctorBtn) {
        showToast("جاري فتح ملف الدكتور...", "info", "bi-person-badge");
        return;
    }

    const secretaryBtn = event.target.closest(".open-secretary-btn");
    if (secretaryBtn) {
        showToast("جاري فتح ملف السكرتير...", "info", "bi-person-workspace");
        return;
    }

    const patientBtn = event.target.closest(".open-patient-btn");
    if (patientBtn) {
        const id = patientBtn.dataset.id;
...
function searchDoctors(value) {
    return adminDB.doctors.filter(doctor =>
        doctor.name.includes(value) || doctor.specialty.includes(value)
    );
}

function searchSecretaries(value) {
    return adminDB.secretaries.filter(secretary =>
        secretary.name.includes(value) || secretary.phone.includes(value)
    );
}
...
function handleSearch() {
    ...
    switch (state.currentPage) {
        case "doctors": {
            ...
            filtered.length === 0 ? renderEmptyState(doctorsTableBody, 5) : renderDoctors(filtered);
            break;
        }
        case "secretaries": {
            const filtered = searchSecretaries(value);
            filtered.length === 0 ? renderEmptyState(secretariesTableBody, 5) : renderSecretaries(filtered);
            break;
        }
        case "patients": {
...
addDoctorBtn?.addEventListener("click", () => openModal(doctorModal));
addSecretaryBtn?.addEventListener("click", () => openModal(secretaryModal));
closeDoctorModalBtn?.addEventListener("click", () => closeModal(doctorModal));
closeSecretaryModalBtn?.addEventListener("click", () => closeModal(secretaryModal));
...
secretaryModal?.addEventListener("click", (event) => {
    if (event.target === secretaryModal) closeModal(secretaryModal);
});
...
addDoctorForm?.addEventListener("submit", function (event) {
    ...
    clearDoctorErrors();
    closeModal(doctorModal);
    showToast(`تمت إضافة الدكتور ${newDoctor.name} بنجاح`, "success", "bi-person-plus-fill");
});
...
addSecretaryForm?.addEventListener("submit", function (event) {
    ...
    clearSecretaryErrors();
    closeModal(secretaryModal);
    showToast(`تمت إضافة السكرتير ${newSecretary.name} بنجاح`, "success", "bi-person-plus");
});
